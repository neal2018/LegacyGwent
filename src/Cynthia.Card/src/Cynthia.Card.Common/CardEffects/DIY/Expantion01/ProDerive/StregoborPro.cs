...
namespace Cynthia.Card
{
    ...
    {//休战：双方各抽1张单位牌，将其战力设为1。
        ...
        public override async Task<int> CardPlayEffect(bool isSpying,bool isReveal)
        {
            ...
            var list = Game.PlayersDeck[Card.PlayerIndex].Where(x => x.Status.Type == CardType.Unit).Take(2).ToList();
            //让玩家(另一半场)选择一张卡,不能不选
            var result = await Game.GetSelectMenuCards(Card.PlayerIndex, list, isCanOver: false);
