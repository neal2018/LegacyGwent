...
namespace Cynthia.Card
{
    ...
    {//择一：创造己方起始牌组中的1张非“密探”牌；或将3张随机非同名手牌变为“解梦术”。
        ...
        public override async Task<int> CardUseEffect()
        {
            ...
            if (switchCard == 0)
            {
                ...
                return await Card.CreateAndMoveStay(list.Where(x=>!x.HasAnyCategorie(Categorie.Agent)).Select(x => x.CardId).Mess(RNG).Take(3).ToArray());
            }
            if (switchCard == 1)
            {
				...
				{
                    await card.Effect.Transform(CardId.LandofaThousandFables2, Card, isForce:true);
				}
            }
            return 0;
        }
